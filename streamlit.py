...
def upload_page():
    ...
    if uploaded_file and st.button("Share", type="primary"):
        with st.spinner("Uploading..."):
            ...
            response = requests.post("http://localhost:8000/uploads", files=files, data=data, headers=get_headers())
...
def feed_page():
    ...
    response = requests.get("http://localhost:8000/feeds", headers=get_headers())
    if response.status_code == 200:
